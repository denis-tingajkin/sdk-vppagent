...
func TestClosingOpeningMemifProxy(t *testing.T) {
	p, err := proxy.NewWithCustomNetwork("source.sock", "target.sock", "unix", nil)
	require.Nil(t, err)
	for i := 0; i < 10; i++ {
		err = p.Start()
		require.Nil(t, err)
...
func TestTransferBetweenMemifProxies(t *testing.T) {
	for i := 0; i < 10; i++ {
		p1, err := proxy.NewWithCustomNetwork("source.sock", "target.sock", "unix", nil)
		require.Nil(t, err)
		p2, err := proxy.NewWithCustomNetwork("target.sock", "source.sock", "unix", nil)
		require.Nil(t, err)
		err = p1.Start()
		require.Nil(t, err)
		err = p2.Start()
